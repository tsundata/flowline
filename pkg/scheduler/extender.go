...
// FailedWorkersMap represents the filtered out nodes, with node names and failure messages
...
func (h *HTTPExtender) Filter(stage *meta.Stage, workers []*meta.Worker) (filteredWorkers []*meta.Worker, failedNodesMap map[string]string, failedAndUnresolvable map[string]string, err error) {
	var (
	...
	if h.nodeCacheCapable && result.WorkerNames != nil {
...
func (h *HTTPExtender) Prioritize(stage *meta.Stage, workers []*meta.Worker) (hostPriorities *framework.HostPriorityList, weight int64, err error) {
	...
	if h.nodeCacheCapable {
		...
		for _, node := range workers {
			workerNameSlice = append(workerNameSlice, node.UID)
		}
		workerNames = &workerNameSlice
	} else {
		workerList = &meta.WorkerList{}
...
func (h *HTTPExtender) ProcessPreemption(stage *meta.Stage, nodeNameToVictims map[string]interface{}, workerInfos interface{}) (map[string]interface{}, error) {
	return nil, nil
}

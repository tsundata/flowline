...
func (r *withRetry) IsNextRetry(_ context.Context, _ *Request, httpReq *http.Request, resp *http.Response, err error, f IsRetryableErrorFunc) bool {
...
func (r *withRetry) Before(ctx context.Context, request *Request) error {
	...
	if r.retryAfter == nil {
		...
		if request.backoff != nil {
			request.backoff.Sleep(request.backoff.CalculateBackoff(u))
		}
		return nil
	}
	...
	flog.Infof("Got a Retry-After %s response for attempt %d to %v", r.retryAfter.Wait, r.retryAfter.Attempt, request.URL().String())
	return nil
}

func (r *withRetry) After(_ context.Context, request *Request, resp *http.Response, err error) {

...
type watchChan struct {
	...
	recursive         bool
	progressNotify    bool
	internalPred      meta.SelectionPredicate
	ctx               context.Context
...
func (w *watcher) Watch(ctx context.Context, key string, rev int64, recursive, progressNotify bool, pred meta.SelectionPredicate) (watch.Interface, error) {
...
func (w *watcher) createWatchChan(ctx context.Context, key string, rev int64, recursive, progressNotify bool, pred meta.SelectionPredicate) *watchChan {
	wc := &watchChan{
		watcher:           w,
		key:               key,
		initialRev:        rev,
	...
	if pred.Empty() {
		...
		wc.internalPred = meta.Everything
	}
...
func logWatchChannelErr(err error) {
	if !strings.Contains(err.Error(), "mvcc: required revision has been compacted") {
		flog.Errorf("watch chan error: %v", err)
	} else {
		flog.Warnf("watch chan error: %v", err)
	}
}
...
func (wc *watchChan) startWatching(watchClosedCh chan struct{}) {
	if wc.initialRev == 0 {
		if err := wc.sync(); err != nil {
			flog.Errorf("failed to sync with latest state: %v", err)
			wc.sendError(err)
			return
		}
	}
	...
	for {
		...
		case e := <-wc.incomingEventChan:
			...
			if len(wc.resultChan) == outgoingBufSize {
				flog.Infof("Fast watcher, slow processing. Probably caused by slow dispatching events to watchers, %d %s", outgoingBufSize, wc.watcher.objectType)
			}
...
func (wc *watchChan) transform(e *event) (res *watch.Event) {
	...
	if err != nil {
		flog.Errorf("failed to prepare current and previous objects: %v", err)
		wc.sendError(err)
		return nil
	}
	...
	case e.isProgressNotify:
		...
		if err := wc.watcher.versioner.UpdateObject(object, uint64(e.rev)); err != nil {
			flog.Errorf("failed to propagate object version: %v", err)
			return nil
		}
		res = &watch.Event{
			Type:   watch.Bookmark,
...
func (wc *watchChan) sendEvent(e *event) {
	if len(wc.incomingEventChan) == incomingBufSize {
		flog.Infof("Fast watcher, slow processing. Probably caused by slow decoding, user not receiving fast, or other processing logic, %d %v", incomingBufSize, wc.watcher.objectType)
	}
	select {

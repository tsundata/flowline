...
// modifications.
//
// consider merging keyLister with this object, tracking a list of
...
func (f *DeltaFIFO) queueActionLocked(actionType DeltaType, obj interface{}) error {
	...
	if len(newDeltas) > 0 {
		...
		if oldDeltas == nil {
			flog.Errorf("Impossible dedupDeltas for id=%q: oldDeltas=%#+v, obj=%#+v; ignoring", id, oldDeltas, obj)
			return nil
		}
...
func (f *DeltaFIFO) Pop(process PopProcessFunc) (interface{}, error) {
	...
	for {
		...
		if !ok {
			...
			flog.Errorf("Inconceivable! %q was in f.queue but not f.items; ignoring.", id)
			continue
		}
		delete(f.items, id)
		...
		if depth > 10 {
			flog.Warnf("DeltaFIFO depth %d", depth)
		}
		err := process(item)
...
func (f *DeltaFIFO) Replace(list []interface{}, _ string) error {
	...
	for _, k := range knownKeys {
		...
		if err != nil {
			deletedObj = nil
			flog.Errorf("Unexpected error %v during lookup of key %v, placing DeleteFinalStateUnknown marker without object", err, k)
		} else if !exists {
			deletedObj = nil
			flog.Infof("Key %v does not exist in known objects store, placing DeleteFinalStateUnknown marker without object", k)
		}
		queuedDeletions++
		if err := f.queueActionLocked(Deleted, DeletedFinalStateUnknown{k, deletedObj}); err != nil {
			return err
...
func (f *DeltaFIFO) syncKeyLocked(key string) error {
	...
	if err != nil {
		flog.Errorf("Unexpected error %v during lookup of key %v, unable to queue object for sync", err, key)
		return nil
	} else if !exists {
		flog.Infof("Key %v does not exist in known objects store, unable to queue object for sync", key)
		return nil
	}
